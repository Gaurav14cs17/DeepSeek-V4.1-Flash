"""V4-Flash transformer blocks: SWA | CSA | HCA + MoE + multi-pass mHC."""
...
from .config import AttnKind, ToyConfig
from .csa import CSA
from .hca import HCA
...
class TransformerBlock(nn.Module):
    def __init__(self, cfg: ToyConfig, layer_idx: int, kind: AttnKind):
        super().__init__()
    ...
    def forward(
        ...
        if self.mhc is not None and mhc_streams is not None:
            # multi-pass: predict A from current streams, then mix (older V4 order)
